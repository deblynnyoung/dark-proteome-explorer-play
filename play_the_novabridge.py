...
def render_ending():
    ...
    col1, col2 = st.columns(2)
    with col1:
        sus_color = "#ef4444" if suspicion >= 60 else "#f59e0b" if suspicion >= 30 else "#22c55e"
        ...
        st.caption("NovaBridge Loyalty Score — Lower is better (for them)")
        if suspicion >= 60:
    ...
    if kind == "stayed":
        st.balloons() if moral > 0 else None
        st.markdown("""
